...
class CaddyViewController: UIViewController, MKMapViewDelegate, CaddyController, CLLocationManagerDelegate {
    
    
    
    
    @IBOutlet weak var callCaddy: UIButton!
    ...
    @IBAction func setLoc(_ sender: Any) {
        if CaddyViewController.apptLocation != nil {
            if canCallCaddy {
                CaddyHandler.Instance.requestCaddy(latitude: Double(CaddyViewController.apptLocation!.latitude), longitude: Double(CaddyViewController.apptLocation!.longitude), time: timeAppt, user: (FIRAuth.auth()?.currentUser?.uid)!)
                confirmationViewController.apptTime = timeAppt
...
extension CaddyViewController: HandleMapSearch {
    func dropPinZoomIn(placemark:MKPlacemark){
        // cache the pin
        ...
        CaddyViewController.apptLocation = annotation.coordinate
        print("MARK")
